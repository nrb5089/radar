...
import numpy as np
from main import ButterFilter, Cheby1Filter, FIR
...
class Receiver:
	def __init__(self,
		...
		self.adc_filter = Cheby1Filter(N = 5,rp = 2, Wn = self.fc_if + self.rf_bw/2,fs = self.Fs_if, btype = 'low')
		self.adc_filter = Cheby1Filter(N = 5,rp = 2, Wn = self.Fs_if/2 -1,fs = self.Fs_if, btype = 'low')
